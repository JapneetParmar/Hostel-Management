...
class deletelol:
    ...
    def __init__(self,pwindow):
        ...
        font1=("Anton",20)
        color1="#81CCFF"
        self.MHD=Label(self.window,text="Delete Record",font=("Franklin Gothic Medium",50,"bold"),background="#F03838",foreground="white",relief=FLAT)
        ...
        self.B4=Button(self.window,text="Delete",font=("Anton",20,'bold'),background="#F03838",foreground="white",relief=RAISED,command=self.deleteData)
    ...
    def deleteData(self):
        ...
        if ans =="yes":
            ...
            if self.oldname==self.default_img_name: 
                ...
                import os
                os.remove("guestpic//"+self.oldname)
            try:
                qry = "delete from newcustomers where Allotted_Room_no=%s"
                rowcount = self.curr.execute(qry , (self.t7.get()))
                self.connection.commit()
                if rowcount==1:
                    messagebox.showinfo("Success","Guest Record Deleted Successfully",parent=self.window)
                    self.ClearData()
            except Exception as e:
                messagebox.showerror("Error","Deletion Error : \n"+str(e),parent=self.window)
            if self.v1.get()=="Creative Room":
                fd=open("CreativeRoom.txt","r")
                d=fd.read()
                fd.close()
                m=d.split("\n")
                s="\n".join(m[:-1])
                fd=open("CreativeRoom.txt","w+")
